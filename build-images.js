  ...
  return new Promise((resolve, reject) => {
    fs.createReadStream(csvFilePath)
      ...
      .on("end", () => {
        ...
        console.log(
          "CSV file processing completed. Found " +
            products.length +
            " products."
        );
        resolve(products);
...
async function processProduct(identifier, catalogType, folderConfig) {
  ...
  const destinationBasePath = path.join(
    BUILD_BASE_PATH,
    folderConfig.destinationBasePath,
    catalogType
  );
...
async function processFolders() {
  ...
  for (const folderConfig of config.folders) {
    ...
    for (const catalogType of CATALOG_TYPES) {
      ...
      if (
        !folderConfig.destinationBasePath ||
        folderConfig.destinationBasePath.length <= 2
      )
        break;
      ...
      for (const product of products) {
        ...
        if (folderConfig.type === "collection") {
          ...
          folderConfig.type === "cover" ||
          folderConfig.type === "logo"
        ) {
      ...
      const maxWidth =
        folderConfig[catalogType] && folderConfig[catalogType].maxWidth;
      const minHeight =
        folderConfig[catalogType] && folderConfig[catalogType].minHeight;
