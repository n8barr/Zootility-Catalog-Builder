...
const collectionBlurbs = JSON.parse(
  fs.readFileSync("collection_blurbs.json", "utf8")
);

function buildCollectionSummary(product, collectionProducts, catalogStyle) {
  // Get the collection blurb if there is one
  const collectionBlurb = collectionBlurbs[product.type];
  ...
  if (collectionBlurb) {
    ...
    gradientStart =
      collectionProducts.length >= 14
        ? 100
        : 10 + collectionProducts.length * 5;
  }
